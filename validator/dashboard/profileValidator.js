...
module.exports = [
    ...
    .isLength({
        max: 50
    }).withMessage('Name can not be more than 50 chars').trim(),
    body('title')
    .not().isEmpty().withMessage('Title Can Not Be Empty')
    .isLength({
        max: 100
    }).withMessage('Title can not be more than 100 chars').trim(),
    body('bio')
    .not().isEmpty().withMessage('Bio Can Not Be Empty')
    .isLength({
        max: 500
    }).withMessage('Bio can not be more than 50 chars').trim(),
    body('website')
